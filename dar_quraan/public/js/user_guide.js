(() => {
  const initialiseGuide = () => {
    ...
    const activeLanguage =
      window.frappe?.boot?.lang || document.documentElement.lang || "";
    const isArabicGuide =
      ...
      selectedIds.forEach((id, sectionIndex) => {
        ...
        section.querySelectorAll("h3").forEach((subheading, subIndex) => {
          subheading.textContent =
            `${sectionNumber}.${subIndex + 1} ${subheading.dataset.originalTitle}`;
        });
      });
    ...
    const doctypeGroups = [
      ["الطلاب والقبول", [["طلبات القبول", "Dar Quraan Admission Application"], ["الطلاب", "Dar Quraan Student"], ["إسنادات الطلاب", "Dar Quraan Student Assignment"], ["المواعيد الأسبوعية", "Dar Quraan Student Weekly Slot"]]],
      ["التعليم اليومي", [["الجلسات", "Dar Quraan Session"], ["الحضور", "Dar Quraan Attendance"], ["تقدم الطالب", "Dar Quraan Student Progress"], ["حالة القرآن", "Dar Quraan Student Quran State"], ["التكليف التالي", "Dar Quraan Next Assignment"], ["تقدم المتون", "Dar Quraan Text Progress"]]],
      ["الجودة والرعاية", [["الاستثناءات", "Dar Quraan Exception"], ["متابعة المعلم", "Dar Quraan Teacher Follow Up"], ["زيارات الإشراف", "Dar Quraan Supervision Visit"], ["الحوادث الانضباطية", "Dar Quraan Discipline Incident"]]],
      ["الاختبارات والشهادات", [["التقييمات", "Dar Quraan Evaluation"], ["الاختبارات الرسمية", "Dar Quraan Exam"], ["الإجازات", "Dar Quraan Ijazah"]]],
      ["التنظيم والإعداد", [["الفروع", "Dar Quraan Branch"], ["مواقع التدريس", "Dar Quraan Teaching Location"], ["المعلمون", "Dar Quraan Teacher"], ["الحلقات", "Dar Quraan Halaqa"], ["البرامج", "Dar Quraan Program"], ["الإعدادات", "Dar Quraan Settings"]]],
    ];

    const routeFor = (doctype) =>
