...
class DarQuraanEvaluation(Document):
	...
	def validate_evaluation_date(self):
		if not self.evaluation_date:
			frappe.throw(_("Evaluation Date is required."))
	...
	def validate_evaluation_type(self):
		...
		if not self.evaluation_type:
			frappe.throw(_("Evaluation Type is required."))

		if self.evaluation_type not in allowed_types:
			frappe.throw(_("Invalid Evaluation Type."))

	def validate_evaluation_items(self):
		items = self.get("evaluation_items") or []

		if not items:
			frappe.throw(_("At least one Evaluation Item " "is required."))
		...
		for index, item in enumerate(
			...
			try:
				...
				frappe.throw(
					_("Evaluation Item, row {0}: {1}").format(
						item.idx or index,
						str(exc),
					)
				)
	...
	def get_evaluation_item_document(
		...
		child = frappe.get_doc(item_data)
	...
	def calculate_overall_score(self):
		...
		for item in items:
			total_score += flt(item.get_score())
