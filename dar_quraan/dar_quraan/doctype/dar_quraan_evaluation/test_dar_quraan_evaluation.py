...
class TestDarQuraanEvaluation(FrappeTestCase):
		...
		return frappe._dict(
			{
				...
				"get_score": lambda: round(
					(memorization_quality * 0.40 + tajweed * 0.30 + fluency * 0.30) / 5 * 100,
					2,
				),
			}
		)
	...
	def make_evaluation(self, **kwargs):
		values = {
			...
			"student_assignment": "TEST-ASSIGNMENT",
			"evaluation_date": "2026-08-10",
			"evaluation_type": "Monthly",
			"evaluation_items": [self.make_item()],
			"status": "Draft",
		}

		values.update(kwargs)

		return DarQuraanEvaluation(values)
	...
	def validate_evaluation(
		...
		def fake_exists(doctype, name=None, *args, **kwargs):
			if doctype == "Dar Quraan Student Assignment" and name == "TEST-ASSIGNMENT":
	...
	def test_student_assignment_required(self):
		evaluation = self.make_evaluation(student_assignment=None)

		with self.assertRaises(frappe.ValidationError):
			self.validate_evaluation(evaluation)

	def test_invalid_student_assignment(self):
		evaluation = self.make_evaluation(student_assignment="INVALID")
	...
	def test_evaluation_date_required(self):
		evaluation = self.make_evaluation(evaluation_date=None)

		with self.assertRaises(frappe.ValidationError):
			self.validate_evaluation(evaluation)

	def test_evaluation_type_required(self):
		evaluation = self.make_evaluation(evaluation_type=None)

		with self.assertRaises(frappe.ValidationError):
			self.validate_evaluation(evaluation)

	def test_invalid_evaluation_type(self):
		evaluation = self.make_evaluation(evaluation_type="Unknown")
	...
	def test_all_evaluation_types_are_valid(self):
		...
		for evaluation_type in evaluation_types:
			evaluation = self.make_evaluation(evaluation_type=evaluation_type)

			self.validate_evaluation(evaluation)

	def test_evaluation_item_required(self):
		evaluation = self.make_evaluation(evaluation_items=[])
	...
	def test_default_status(self):
		evaluation = self.make_evaluation(status=None)
	...
	def test_invalid_status_rejected(self):
		evaluation = self.make_evaluation(status="Invalid")
