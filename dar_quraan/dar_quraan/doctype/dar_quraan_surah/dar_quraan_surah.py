...
class DarQuraanSurah(Document):
    ...
    def validate_surah_number(self):
        ...
        if surah_number < 1 or surah_number > 114:
            frappe.throw(
                _("Surah Number must be between 1 and 114.")
            )
    ...
    def validate_ayah_count(self):
        ...
        if ayah_count < 1:
            frappe.throw(
                _("Ayah Count must be greater than zero.")
            )
    ...
    def validate_page_range(self):
        ...
        if start_page < 1 or start_page > 604:
            frappe.throw(
                _("Start Page must be between 1 and 604.")
            )

        if end_page < 1 or end_page > 604:
            frappe.throw(
                _("End Page must be between 1 and 604.")
            )

        if end_page < start_page:
            frappe.throw(
                _("End Page cannot be before Start Page.")
            )
    ...
    def validate_juz_range(self):
        ...
        if start_juz < 1 or start_juz > 30:
            frappe.throw(
                _("Start Juz must be between 1 and 30.")
            )

        if end_juz < 1 or end_juz > 30:
            frappe.throw(
                _("End Juz must be between 1 and 30.")
            )

        if end_juz < start_juz:
            frappe.throw(
                _("End Juz cannot be before Start Juz.")
            )