...
class DarQuraanAcademicYear(Document):
	...
	def validate_unique_name(self):
		...
		if existing:
			frappe.throw(_("Academic Year {0} already exists.").format(frappe.bold(self.academic_year_name)))
	...
	def validate_current_year(self):
		...
		if self.disabled:
			frappe.throw(_("A disabled Academic Year cannot be marked as current."))
