...
class DarQuraanJuzLevel(Document):
    ...
    def validate_juz_values(self):
        ...
        for label, value in values.items():
            ...
            if numeric_value < 0 or numeric_value > 30:
                frappe.throw(
                    _("{0} must be between 0 and 30.").format(label)
                )
    ...
    def validate_range(self):
        ...
        if minimum_juz > maximum_juz:
            frappe.throw(
                _("Minimum Juz cannot be greater than Maximum Juz.")
            )
    ...
    def validate_juz_count_within_range(self):
        ...
        if juz_count < minimum_juz or juz_count > maximum_juz:
            frappe.throw(
                _(
                    "Juz Count must be between Minimum Juz {0} "
                    "and Maximum Juz {1}."
                ).format(
    ...
    def validate_unique_level_name(self):
        ...
        if existing_level:
            frappe.throw(
                _("Juz Level {0} already exists.").format(
                    frappe.bold(self.level_name)
                )
            )
    ...
    def validate_unique_display_order(self):
        ...
        if existing_level:
            frappe.throw(
                _(
                    "Display Order {0} is already assigned to Juz Level {1}."
                ).format(
    ...
    def validate_overlapping_range(self):
        ...
        if overlapping_level:
            ...
            frappe.throw(
                _(
                    "The range {0}–{1} overlaps with Juz Level {2}, "
                    "which uses the range {3}–{4}."
                ).format(
                    frappe.bold(minimum_juz),
                    frappe.bold(maximum_juz),
