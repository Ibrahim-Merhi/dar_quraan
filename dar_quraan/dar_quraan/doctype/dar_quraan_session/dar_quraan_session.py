...
class DarQuraanSession(Document):
	...
	def fetch_halaqa_context(self):
		...
		if not self.branch:
			frappe.throw(_("Halaqa {0} does not have a Branch.").format(self.halaqa))

		if not self.teaching_location:
			frappe.throw(_("Halaqa {0} does not have a " "Teaching Location.").format(self.halaqa))

		if not self.academic_year:
			frappe.throw(_("Halaqa {0} does not have an " "Academic Year.").format(self.halaqa))

		if not self.academic_term:
			frappe.throw(_("Halaqa {0} does not have an " "Academic Term.").format(self.halaqa))

		if not self.teacher:
			frappe.throw(_("Halaqa {0} does not have a Mentor.").format(self.halaqa))

		if halaqa.status == "Closed":
			frappe.throw(_("Sessions cannot be created for a Closed Halaqa."))
	...
	def validate_session_date(self):
		...
		if halaqa.start_date:
			if session_date < getdate(halaqa.start_date):
				frappe.throw(_("Session Date cannot be before " "the Halaqa Start Date."))

		if halaqa.end_date:
			if session_date > getdate(halaqa.end_date):
				frappe.throw(_("Session Date cannot be after " "the Halaqa End Date."))

		self.validate_session_date_against_term(session_date)
	...
	def validate_session_date_against_term(
		...
		if not term:
			frappe.throw(_("Selected Academic Term does not exist."))

		if term.status != "Open":
			frappe.throw(_("Academic Term must be Open."))

		if term.start_date:
			if session_date < getdate(term.start_date):
				frappe.throw(_("Session Date cannot be before " "the Academic Term starts."))

		if term.end_date:
			if session_date > getdate(term.end_date):
				frappe.throw(_("Session Date cannot be after " "the Academic Term ends."))
	...
	def validate_timing(self):
		...
		if not self.start_time:
			frappe.throw(_("Start Time is required when End Time is set."))

		if not self.end_time:
			frappe.throw(_("End Time is required when Start Time is set."))
	...
	def validate_session_type(self):
		...
		if self.session_type not in allowed_types:
			frappe.throw(_("Session Type must be Regular, Makeup, " "Extra, or Evaluation."))
	...
	def validate_status(self):
		...
		if self.status not in allowed_statuses:
			frappe.throw(_("Status must be Planned, In Progress, " "Completed, or Cancelled."))
