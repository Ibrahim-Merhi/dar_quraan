...
class DarQuraanAttendance(Document):
    ...
    def validate_session(self):
        ...
        if not frappe.db.exists(
            ...
            frappe.throw(
                _("Selected Session does not exist.")
            )
    ...
    def fetch_session_context(self):
        ...
        if not self.attendance_date:
            frappe.throw(
                _(
                    "Selected Session does not have "
                    "a Session Date."
                )
            )
    ...
    def populate_attendance_items_if_empty(self):
        ...
        assignments = (
            self.get_active_student_assignments()
        )
    ...
    def validate_attendance_items(self):
        ...
        for index, row in enumerate(
            ...
            row = (
                self.get_attendance_item_document(
                    row,
                    index,
                )
            )
            ...
            if (
                ...
                frappe.throw(
                    _(
                        "Student Assignment {0} appears "
                        "more than once in Attendance."
                    ).format(
                        row.student_assignment
                    )
                )
    ...
    def validate_item_assignment(
        ...
        if not frappe.db.exists(
            ...
            frappe.throw(
                _(
                    "Student Assignment {0} does not exist."
                ).format(
                    row.student_assignment
                )
            )
        ...
        if assignment.student != row.student:
            frappe.throw(
                _(
                    "Student in row {0} does not match "
        ...
        if assignment.halaqa != self.halaqa:
            frappe.throw(
                _(
                    "Student Assignment in row {0} does not "
                    "belong to this Halaqa."
                ).format(
                    row.idx or 1
                )
            )
