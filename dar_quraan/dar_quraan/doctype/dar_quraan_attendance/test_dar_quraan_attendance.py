...
class TestDarQuraanAttendance(FrappeTestCase):
	...
	def validate_attendance(
		...
		def fake_get_doc(
			...
			**kwargs,
			...
			if len(args) >= 2 and args[0] == "Dar Quraan Session" and args[1] == "DQ-SES-TEST-00001":
				return session
		...
		def fake_exists(
			...
			**kwargs,
			...
			if doctype == "Dar Quraan Session":
				if session_exists and filters == "DQ-SES-TEST-00001":
		...
		def fake_get_meta(
			...
			**kwargs,
			...
			if doctype == "Dar Quraan Student Assignment":
				...
				fake_meta.has_field = lambda fieldname: fieldname in {
					"status",
					"student_name",
				}
	...
	def test_valid_attendance(self):
		...
		self.validate_attendance(attendance)
	...
	def test_students_are_auto_populated(self):
		...
		self.assertEqual(
			attendance.attendance_items[0].student_assignment,
			"DQ-SA-TEST-001",
		)

		self.assertEqual(
			attendance.attendance_items[0].attendance_status,
			"Present",
		)
