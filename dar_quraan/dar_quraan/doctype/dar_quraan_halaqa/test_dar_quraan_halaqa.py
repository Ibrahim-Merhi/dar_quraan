...
class TestDarQuraanHalaqa(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.term_start = getdate(
            add_days(cls.year_start, 10)
        )
        cls.term_end = getdate(
            add_days(cls.year_start, 180)
        )
        ...
        cls.academic_year = cls.create_academic_year(
            academic_year_name=(
                f"Test Academic Year {cls.test_suffix}"
            ),
            status="Open",
        ...
        cls.closed_academic_year = cls.create_academic_year(
            academic_year_name=(
                f"Closed Academic Year {cls.test_suffix}"
            ),
            status="Closed",
            start_date=cls.year_start,
            end_date=cls.year_end,
        ...
        cls.other_academic_year = cls.create_academic_year(
            academic_year_name=(
                f"Other Academic Year {cls.test_suffix}"
            ),
            status="Open",
            start_date=cls.year_start,
            end_date=cls.year_end,
    ...
    def test_same_name_is_allowed_in_different_branch(
        ...
        second_halaqa = self.create_halaqa(
            ...
            teaching_location=(
                self.other_branch_location.name
            ),
        )
    ...
    def test_location_that_does_not_allow_halaqas_is_rejected(
        ...
        with self.assertRaisesRegex(
            ...
            self.create_halaqa(
                ...
                teaching_location=(
                    self.location_not_allowing_halaqas.name
                ),
            )
    ...
    def test_academic_year_must_be_open(self):
        with self.assertRaisesRegex(
            ...
            self.create_halaqa(
                ...
                academic_year=(
                    self.closed_academic_year.name
                ),
            )
    ...
    def test_academic_term_must_be_open(self):
        with self.assertRaisesRegex(
            ...
            self.create_halaqa(
                ...
                halaqa_name=self.name(
                    "Closed Academic Term"
                ),
                academic_term=(
                    self.closed_academic_term.name
                ),
            )
        ...
        doc = frappe.get_doc(
            {
                ...
                "academic_year": (
                    academic_year
                    or self.academic_year.name
                ),
                "academic_term": (
                    academic_term
                    or self.academic_term.name
                ),
                "mentor": mentor,
                "assistant_mentor": (
                    assistant_mentor
        ...
        return frappe.get_doc(
            {
                ...
                "city": "Test City",
                "address": (
                    "Test Teaching Location Address"
                ),
                "capacity": 30,
                "allow_halaqas": (
                    allow_halaqas
