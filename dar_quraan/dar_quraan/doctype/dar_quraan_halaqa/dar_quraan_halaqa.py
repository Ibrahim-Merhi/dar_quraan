...
class DarQuraanHalaqa(Document):
    ...
    def validate_halaqa_code(self):
        ...
        if not re.fullmatch(r"[A-Z0-9_-]+", self.halaqa_code):
            frappe.throw(
                _(
                    "Halaqa Code may contain only uppercase letters, numbers, hyphens and underscores."
                )
            )
    ...
    def validate_unique_halaqa_code(self):
        ...
        if existing:
            frappe.throw(
                _("Halaqa Code {0} already exists.").format(
                    frappe.bold(self.halaqa_code)
                )
            )
    ...
    as_dict=True,
        ...
        if location.branch != self.branch:
            frappe.throw(
                _("Teaching Location must belong to the selected Branch.")
            )
    ...
    def validate_academic_term(self):
        ...
        if term.academic_year != self.academic_year:
            frappe.throw(
                _("Academic Term must belong to the selected Academic Year.")
            )

    def validate_capacity(self):
        if cint(self.maximum_students) <= 0:
            frappe.throw(
                _("Maximum Students must be greater than zero.")
            )

        if cint(self.current_students) < 0:
            frappe.throw(
                _("Current Students cannot be negative.")
            )

        if cint(self.current_students) > cint(self.maximum_students):
            frappe.throw(
                _("Current Students cannot exceed Maximum Students.")
            )
        ...
        if self.academic_term and self.start_date:
            ...
            if term:
                if self.start_date < term.start_date:
                    frappe.throw(
                        _("Start Date cannot be before the Academic Term starts.")
                    )

                if self.end_date and self.end_date > term.end_date:
                    frappe.throw(
                        _("End Date cannot be after the Academic Term ends.")
                    )
    ...
    def validate_mentors(self):
        if (
            self.mentor
            and self.assistant_mentor
            and self.mentor == self.assistant_mentor
        ):
            frappe.throw(
                _("Mentor and Assistant Mentor cannot be the same person.")
            )
    ...
    def apply_status_rules(self):
        if self.status == "Closed":
            ...
            if not self.closed_on:
                frappe.throw(
                    _("Closed On is required when the Halaqa is Closed.")
                )