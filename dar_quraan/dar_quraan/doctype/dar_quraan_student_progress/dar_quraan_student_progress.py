...
class DarQuraanStudentProgress(Document):
	...
	def validate_progress_date(self):
		if not self.progress_date:
			frappe.throw(_("Progress Date is required."))
	...
	def validate_progress_items(self):
		progress_items = self.get("progress_items") or []
		...
		for index, row in enumerate(
			...
			try:
				...
				frappe.throw(
					_("Progress Item, row {0}: {1}").format(
						row.idx or index,
						str(exc),
					)
				)

			self.validate_progress_item_against_assignment(row)
	...
	def validate_progress_item_against_assignment(
		...
		if progress_type == "New Memorization":
			assigned_ranges = self.get("new_memorization") or []

		elif progress_type == "Revision":
			assigned_ranges = self.get("revision") or []

		else:
			frappe.throw(
				_("Invalid Progress Type in Progress Item " "row {0}.").format(
					self.get_row_value(
						item,
						"idx",
					)
		...
		frappe.throw(
			_("Progress Item row {0} does not fall inside " "any assigned {1} Quran range.").format(
				self.get_row_value(
					item,
					"idx",
				)
