...
class DarQuraanNextAssignment(Document):
	...
	def validate_assignment_date(self):
		if not self.assignment_date:
			frappe.throw(_("Assignment Date is required."))

	def validate_quran_work(self):
		new_memorization = self.get("new_memorization") or []

		revision = self.get("revision") or []

		if not new_memorization and not revision:
			frappe.throw(_("At least one New Memorization or " "Revision range is required."))
	...
	def validate_source_progress(self):
		if not frappe.db.exists(
			...
			frappe.throw(_("Selected Source Progress does not exist."))
		...
		if source_assignment and source_assignment != self.student_assignment:
			frappe.throw(_("Source Progress must belong to the same " "Student Assignment."))
	...
	def validate_source_evaluation(self):
		if not frappe.db.exists(
			...
			frappe.throw(_("Selected Source Evaluation does not exist."))
		...
		if source_assignment and source_assignment != self.student_assignment:
			frappe.throw(_("Source Evaluation must belong to the same " "Student Assignment."))
	...
	def generate_from_source_progress(self):
		...
		if not self.source_progress:
			frappe.throw(_("Source Progress is required."))
		...
		if progress.student_assignment != self.student_assignment:
			frappe.throw(_("Source Progress must belong to the same " "Student Assignment."))

		if progress.status != "Completed":
			frappe.throw(_("Source Progress must be Completed before " "generating the next assignment."))

		progress_items = progress.get("progress_items") or []

		if not progress_items:
			frappe.throw(_("Source Progress does not contain any " "Progress Items."))
		...
		total_generated = generated_new_memorization + generated_revision
		...
		return {
			"new_memorization": (generated_new_memorization),
	...
	def validate_status(self):
		...
		if self.status not in allowed_statuses:
			frappe.throw(_("Status must be Draft, Ready, Assigned, " "Completed, or Cancelled."))
