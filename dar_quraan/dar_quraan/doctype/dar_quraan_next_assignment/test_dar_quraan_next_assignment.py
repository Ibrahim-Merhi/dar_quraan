...
class TestDarQuraanNextAssignment(FrappeTestCase):
	...
	def validate_doc(
		...
		def fake_exists(
			...
			**kwargs,
			...
			if doctype == "Dar Quraan Student Assignment" and name == "DQ-SA-TEST-00001":
				return "DQ-SA-TEST-00001"

			if doctype == "Dar Quraan Student Progress" and name == "DQ-PROG-TEST-00001":
				return "DQ-PROG-TEST-00001"

			if doctype == "Dar Quraan Evaluation" and name == "DQ-EVAL-TEST-00001":
		...
		def fake_get_value(
			...
			**kwargs,
			...
			if (
				...
				return source_progress_assignment or "DQ-SA-TEST-00001"
			...
			if (
				...
				return source_evaluation_assignment or "DQ-SA-TEST-00001"
	...
	def generate_from_progress(
		...
		def fake_get_doc(*args, **kwargs):
			if (
				len(args) >= 2
				and args[0] == "Dar Quraan Student Progress"
				and args[1] == "DQ-PROG-TEST-00001"
			):
				return progress
	...
	def test_source_progress_must_match_assignment(self):
		...
		with self.assertRaises(frappe.ValidationError):
			self.validate_doc(
				doc,
				source_progress_assignment=("DQ-SA-OTHER"),
			)
	...
	def test_source_evaluation_must_match_assignment(self):
		...
		with self.assertRaises(frappe.ValidationError):
			self.validate_doc(
				doc,
				source_evaluation_assignment=("DQ-SA-OTHER"),
			)
