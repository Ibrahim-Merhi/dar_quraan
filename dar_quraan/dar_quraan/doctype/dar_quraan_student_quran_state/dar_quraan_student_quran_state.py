...
class DarQuraanStudentQuranState(Document):
	...
	def validate_unique_state(self):
		...
		if existing:
			frappe.throw(
				_("A Quran State already exists for Student " "Assignment {0}.").format(
					self.student_assignment
				)
			)
	...
	def fetch_assignment_context(self):
		...
		if not self.student:
			frappe.throw(
				_("Student Assignment {0} does not have " "a Student.").format(self.student_assignment)
			)

		if not self.teacher:
			frappe.throw(
				_("Student Assignment {0} does not have " "a Teacher.").format(self.student_assignment)
			)

		if not self.halaqa:
			frappe.throw(
				_("Student Assignment {0} does not have " "a Halaqa.").format(self.student_assignment)
			)
	...
	def get_surah_data(
		...
		if not surah:
			frappe.throw(_("Selected Surah {0} does not exist.").format(surah_name))

		if not surah.is_active:
			frappe.throw(_("Selected Surah {0} is inactive.").format(surah_name))
	...
	def validate_current_memorization(self):
		...
		if not self.current_surah:
			frappe.throw(_("Current Surah is required."))

		if self.current_ayah in (None, ""):
			frappe.throw(_("Current Ayah is required."))

		if self.current_page in (None, ""):
			frappe.throw(_("Current Page is required."))

		surah = self.get_surah_data(self.current_surah)

		current_ayah = cint(self.current_ayah)

		current_page = cint(self.current_page)

		if current_ayah < 1:
			frappe.throw(_("Current Ayah must be greater than zero."))

		if current_ayah > cint(surah.ayah_count):
			frappe.throw(
				_("Current Ayah cannot exceed {0} for " "the selected Surah.").format(surah.ayah_count)
			)

		if current_page < cint(surah.start_page):
			frappe.throw(
				_("Current Page cannot be before page {0} " "for the selected Surah.").format(
					surah.start_page
				)
			)

		if current_page > cint(surah.end_page):
			frappe.throw(
				_("Current Page cannot exceed page {0} " "for the selected Surah.").format(surah.end_page)
			)
	...
	def validate_revision_state(self):
		...
		revision_values_present = any(
			[
				self.revision_surah,
				self.revision_from_ayah not in (None, "", 0),
				self.revision_to_ayah not in (None, "", 0),
				self.revision_from_page not in (None, "", 0),
				self.revision_to_page not in (None, "", 0),
			]
		)
		...
		required_fields = {
			...
			"revision_from_ayah": _("Revision From Ayah"),
			"revision_to_ayah": _("Revision To Ayah"),
			"revision_from_page": _("Revision From Page"),
			"revision_to_page": _("Revision To Page"),
		}
		...
		surah = self.get_surah_data(self.revision_surah)

		from_ayah = cint(self.revision_from_ayah)

		to_ayah = cint(self.revision_to_ayah)

		from_page = cint(self.revision_from_page)

		to_page = cint(self.revision_to_page)

		if from_ayah < 1:
			frappe.throw(_("Revision From Ayah must be " "greater than zero."))

		if to_ayah < 1:
			frappe.throw(_("Revision To Ayah must be " "greater than zero."))

		if from_ayah > cint(surah.ayah_count):
			frappe.throw(
				_("Revision From Ayah cannot exceed {0} " "for the selected Surah.").format(surah.ayah_count)
			)

		if to_ayah > cint(surah.ayah_count):
			frappe.throw(
				_("Revision To Ayah cannot exceed {0} " "for the selected Surah.").format(surah.ayah_count)
			)

		if to_ayah < from_ayah:
			frappe.throw(_("Revision To Ayah cannot be before " "Revision From Ayah."))

		if from_page < cint(surah.start_page):
			frappe.throw(
				_("Revision From Page cannot be before " "page {0} for the selected Surah.").format(
					surah.start_page
				)
			)

		if from_page > cint(surah.end_page):
			frappe.throw(
				_("Revision From Page cannot exceed " "page {0} for the selected Surah.").format(
					surah.end_page
				)
			)

		if to_page < cint(surah.start_page):
			frappe.throw(
				_("Revision To Page cannot be before " "page {0} for the selected Surah.").format(
					surah.start_page
				)
			)

		if to_page > cint(surah.end_page):
			frappe.throw(
				_("Revision To Page cannot exceed " "page {0} for the selected Surah.").format(surah.end_page)
			)

		if to_page < from_page:
			frappe.throw(_("Revision To Page cannot be before " "Revision From Page."))
	...
	def calculate_progress_summary(self):
		...
		completed_pages = cint(self.current_page)
	...
	def validate_status(self):
		...
		if self.status not in allowed_statuses:
			frappe.throw(_("Status must be Active, Paused, " "Completed, or Inactive."))
