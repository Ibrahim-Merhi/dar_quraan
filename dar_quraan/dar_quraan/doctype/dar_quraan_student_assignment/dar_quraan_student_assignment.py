...
class DarQuraanStudentAssignment(Document):
	...
	def normalize_values(self):
		self.assignment_notes = self.clean_text(self.assignment_notes)
		self.transfer_reason = self.clean_text(self.transfer_reason)
		self.riwayah = self.clean_text(self.riwayah)

		if self.status:
			self.status = self.status.strip()

		if self.assignment_type:
			self.assignment_type = self.assignment_type.strip()
	...
	def fetch_reference_values(self):
		...
		if self.halaqa:
			...
			if halaqa_values:
				if not self.branch and halaqa_values.branch:
					self.branch = halaqa_values.branch

				if not self.teacher and halaqa_values.mentor:
					self.teacher = halaqa_values.mentor

				if not self.teaching_location and halaqa_values.teaching_location:
					self.teaching_location = halaqa_values.teaching_location
	...
	def set_effective_dates(self):
		...
		if self.start_date:
			self.effective_from = f"{self.start_date} 00:00:00"
		else:
			self.effective_from = None

		if self.end_date:
			self.effective_to = f"{self.end_date} 23:59:59"
		else:
	...
	def validate_student(self):
		...
		if not student:
			frappe.throw(_("Student {0} does not exist.").format(frappe.bold(self.student)))
		...
		if student.status != "Active":
			frappe.throw(
				_(
					...
					frappe.bold(student.student_name or self.student),
					frappe.bold(student.status),
				)
			)

		if cint(student.disabled):
			frappe.throw(_("Disabled students cannot be assigned."))
	...
	def validate_branch(self):
		...
		if not branch:
			frappe.throw(_("Branch {0} does not exist.").format(frappe.bold(self.branch)))

		if branch.status != "Active":
			frappe.throw(
				_("Branch {0} must be Active. Current status: {1}.").format(
	...
	def validate_teaching_location(self):
		...
		if not location:
			frappe.throw(
				_("Teaching Location {0} does not exist.").format(frappe.bold(self.teaching_location))
			)

		if location.status != "Active":
			frappe.throw(
				_("Teaching Location {0} must be Active. " "Current status: {1}.").format(
		...
		if location.branch and location.branch != self.branch:
			frappe.throw(
				_("Teaching Location {0} belongs to Branch {1}, " "not the selected Branch {2}.").format(
					frappe.bold(self.teaching_location),
	...
	def validate_teacher(self):
		...
		if not frappe.db.exists(
			...
			frappe.throw(_("Teacher {0} does not exist.").format(frappe.bold(self.teacher)))
		...
		if teacher_meta.has_field("status"):
			if teacher.get("status") != "Active":
				frappe.throw(
					_("Teacher {0} must be Active. Current status: {1}.").format(
						frappe.bold(self.teacher),
						frappe.bold(teacher.get("status") or _("Not Set")),
					)
				)

		elif teacher_meta.has_field("disabled"):
			if cint(teacher.get("disabled")):
				frappe.throw(_("Teacher {0} is disabled.").format(frappe.bold(self.teacher)))

		elif teacher_meta.has_field("is_active"):
			if not cint(teacher.get("is_active")):
				frappe.throw(_("Teacher {0} must be Active.").format(frappe.bold(self.teacher)))

		elif teacher_meta.has_field("enabled"):
			if not cint(teacher.get("enabled")):
				frappe.throw(_("Teacher {0} must be enabled.").format(frappe.bold(self.teacher)))
		...
		if teacher_meta.has_field("branch"):
			...
			if teacher_branch and self.branch and teacher_branch != self.branch:
				frappe.throw(
					_("Teacher {0} belongs to Branch {1}, " "not the selected Branch {2}.").format(
						frappe.bold(self.teacher),
	...
	def validate_halaqa(self):
		...
		if not halaqa:
			frappe.throw(_("Halaqa {0} does not exist.").format(frappe.bold(self.halaqa)))

		if halaqa.status != "Active":
			frappe.throw(
				_("Halaqa {0} must be Active. Current status: {1}.").format(
		...
		if halaqa.branch and halaqa.branch != self.branch:
			frappe.throw(
				_("Halaqa {0} belongs to Branch {1}, " "not the selected Branch {2}.").format(
					frappe.bold(self.halaqa),
		...
		if halaqa.mentor and halaqa.mentor != self.teacher:
			frappe.throw(
				_("Halaqa {0} is assigned to Teacher {1}, " "not the selected Teacher {2}.").format(
					frappe.bold(self.halaqa),
		...
		if (
			...
			frappe.throw(
				_("Halaqa {0} uses Teaching Location {1}, " "not the selected Teaching Location {2}.").format(
					frappe.bold(self.halaqa),
		...
		if halaqa.academic_year and self.academic_year and halaqa.academic_year != self.academic_year:
			frappe.throw(
				_("Halaqa {0} belongs to Academic Year {1}, " "not {2}.").format(
					frappe.bold(self.halaqa),
		...
		if halaqa.academic_term and self.academic_term and halaqa.academic_term != self.academic_term:
			frappe.throw(
				_("Halaqa {0} belongs to Academic Term {1}, " "not {2}.").format(
					frappe.bold(self.halaqa),
		...
		if self.is_new() and self.status == "Active" and not cint(halaqa.allow_new_enrollments):
			frappe.throw(
				_("Halaqa {0} does not currently allow new " "student assignments.").format(
					frappe.bold(self.halaqa)
				)
			)
	...
	def validate_academic_year(self):
		...
		if (
			...
			frappe.throw(
				_("Assignment Start Date cannot be earlier than " "Academic Year start date {0}.").format(
		...
		if (
			...
			frappe.throw(
				_("Assignment Start Date cannot be later than " "Academic Year end date {0}.").format(
		...
		if (
			...
			frappe.throw(
				_("Assignment End Date cannot be later than " "Academic Year end date {0}.").format(
					frappe.bold(academic_year.end_date)
				)
			)
		...
		if (
			...
			frappe.throw(
				_("Assignment End Date cannot be earlier than " "Academic Year start date {0}.").format(
					frappe.bold(academic_year.start_date)
				)
			)
	...
	def validate_academic_term(self):
		...
		if not academic_term:
			frappe.throw(_("Academic Term {0} does not exist.").format(frappe.bold(self.academic_term)))

		if academic_term.academic_year != self.academic_year:
			frappe.throw(
				_(
					"Academic Term {0} belongs to Academic Year {1}, " "not the selected Academic Year {2}."
				).format(
					frappe.bold(self.academic_term),
		...
		if academic_term.status not in {"Active", "Planned"}:
			frappe.throw(
				_("Academic Term {0} cannot be used because its " "status is {1}.").format(
					frappe.bold(self.academic_term),
		...
		if (
			...
			frappe.throw(
				_("Assignment Start Date cannot be earlier than " "Academic Term start date {0}.").format(
		...
		if (
			...
			frappe.throw(
				_("Assignment Start Date cannot be later than " "Academic Term end date {0}.").format(
		...
		if (
			...
			frappe.throw(
				_("Assignment End Date cannot be later than " "Academic Term end date {0}.").format(
					frappe.bold(academic_term.end_date)
				)
			)
		...
		if (
			...
			frappe.throw(
				_("Assignment End Date cannot be earlier than " "Academic Term start date {0}.").format(
					frappe.bold(academic_term.start_date)
				)
			)
	...
	def validate_assignment_dates(self):
		...
		if self.status == "Completed" and not self.end_date:
			frappe.throw(_("End Date is required for a Completed assignment."))

		if self.status == "Cancelled" and self.is_current_assignment:
			frappe.throw(_("A Cancelled assignment cannot be current."))
	...
	def validate_effective_dates(self):
		...
		if (
			...
			frappe.throw(_("Effective From cannot be earlier than " "the assignment Start Date."))

		if self.end_date and self.effective_to and getdate(self.effective_to) > getdate(self.end_date):
			frappe.throw(_("Effective To cannot be later than " "the assignment End Date."))
	...
	def validate_assignment_type(self):
		...
		if self.assignment_type not in valid_types:
			frappe.throw(_("Assignment Type must be one of: {0}.").format(", ".join(sorted(valid_types))))

		if self.assignment_type == "Transfer" and not self.transfer_reason:
			frappe.throw(_("Transfer Reason is required for a Transfer assignment."))
	...
	def validate_status(self):
		...
		if self.status not in valid_statuses:
			frappe.throw(_("Invalid assignment status: {0}.").format(frappe.bold(self.status)))
	...
	def validate_active_primary_assignment(self):
		...
		if existing_assignment:
			frappe.throw(
				_("Student {0} already has an Active Primary " "Assignment: {1}.").format(
	...
	def validate_overlapping_primary_assignment(self):
		...
		new_end = getdate(self.end_date) if self.end_date else getdate("9999-12-31")
		...
		for assignment in existing_assignments:
			...
			existing_end = getdate(assignment.end_date) if assignment.end_date else getdate("9999-12-31")

			periods_overlap = new_start <= existing_end and new_end >= existing_start

			if periods_overlap:
				frappe.throw(
					_(
						"The assignment period overlaps with Primary " "Assignment {0}, from {1} to {2}."
					).format(
						frappe.bold(assignment.name),
						frappe.bold(assignment.start_date),
						frappe.bold(assignment.end_date or _("Open-ended")),
					)
				)
	...
	def validate_legacy_fields(self):
		...
		if (
			...
			and get_datetime(self.legacy_updated_at) < get_datetime(self.legacy_created_at)
		):
			frappe.throw(_("Legacy Updated At cannot be earlier than " "Legacy Created At."))
		...
		if self.legacy_assignment_id:
			...
			if duplicate:
				frappe.throw(
					_("Legacy Assignment ID {0} is already used by " "Student Assignment {1}.").format(
	...
	def sync_student_defaults(self):
		...
		if self.status == "Active" and self.assignment_type == "Primary" and cint(self.is_current_assignment):
			frappe.db.set_value(
				"Dar Quraan Student",
				self.student,
				{
