...
class DarQuraanStudent(Document):
	...
	def normalize_values(self):
		...
		if original_student_code and not self.student_code:
			frappe.throw(_("Student Code must contain at least one letter or number."))
	...
	def set_student_name(self):
		...
		self.student_name = " ".join(value for value in name_parts if value)
	...
	def validate_student_code(self):
		...
		if len(self.student_code) < 3:
			frappe.throw(_("Student Code must contain at least 3 characters."))

		if not re.fullmatch(r"[A-Z0-9][A-Z0-9\-_]*", self.student_code):
			frappe.throw(
				_("Student Code may contain only uppercase letters, " "numbers, hyphens, and underscores.")
			)
	...
	def validate_unique_student_code(self):
		...
		if existing_student:
			frappe.throw(
				_("Student Code {0} is already used by student {1}.").format(
	...
	def validate_legacy_student_id(self):
		...
		if int(self.legacy_student_id) <= 0:
			frappe.throw(_("Legacy Student ID must be greater than zero."))
		...
		if existing_student:
			frappe.throw(
				_("Legacy Student ID {0} is already linked to student {1}.").format(
	...
	def validate_duplicate_student(self):
		...
		if duplicate:
			frappe.throw(
				_("A student with the same name and Date of Birth " "already exists: {0}.").format(
	...
	def validate_dates(self):
		...
		if self.registration_date:
			...
			if registration_date > today:
				frappe.throw(_("Registration Date cannot be in the future."))

			if self.date_of_birth:
				date_of_birth = getdate(self.date_of_birth)

				if registration_date < date_of_birth:
					frappe.throw(_("Registration Date cannot be earlier than " "Date of Birth."))
	...
	def validate_branch(self):
		...
		if not branch:
			frappe.throw(_("The selected Default Branch does not exist."))

		if branch.status != "Active":
			frappe.throw(
				_("The selected Default Branch must be Active. " "Current status: {0}.").format(
	...
	def validate_teacher(self):
		...
		if not teacher:
			frappe.throw(_("The selected Default Teacher does not exist."))

		if teacher.status != "Active":
			frappe.throw(
				_("The selected Default Teacher must be Active. " "Current status: {0}.").format(
					frappe.bold(teacher.status)
				)
			)
	...
	def validate_halaqa(self):
		...
		if not halaqa:
			frappe.throw(_("The selected Default Halaqa does not exist."))

		if halaqa.status != "Active":
			frappe.throw(
				_("The selected Default Halaqa must be Active. " "Current status: {0}.").format(
					frappe.bold(halaqa.status)
				)
			)

		if self.default_branch and halaqa.branch and halaqa.branch != self.default_branch:
			frappe.throw(
				_("Default Halaqa belongs to Branch {0}, " "not the selected Default Branch {1}.").format(
					frappe.bold(halaqa.branch),
					frappe.bold(self.default_branch),
				)
			)

		if self.default_teacher and halaqa.mentor and halaqa.mentor != self.default_teacher:
			frappe.throw(
				_("Default Halaqa mentor is {0}, " "not the selected Default Teacher {1}.").format(
		...
		if not halaqa.allow_new_enrollments:
			frappe.throw(_("The selected Default Halaqa does not allow " "new student assignments."))
	...
	def validate_quran_profile(self):
		if self.study_track == "Qiraat" and not self.qiraat_method:
			frappe.throw(_("Qiraat Method is required when Study Track is Qiraat."))

		if self.study_track != "Qiraat":
			self.qiraat_method = None

		if self.qiraat_method == "Other" and not self.notes:
			frappe.throw(_("Add the Qiraat method details in Notes " "when Qiraat Method is Other."))

		if not self.has_previous_ijazah:
			self.previous_ijazah_details = None

		if self.has_previous_ijazah and not self.previous_ijazah_details:
			frappe.throw(_("Previous Ijazah Details are required when " "Has Previous Ijazah is enabled."))

		if self.quran_status == "Mujaz" and not self.has_previous_ijazah:
			frappe.throw(_("Has Previous Ijazah must be enabled when " "Quran Status is Mujaz."))
	...
	def apply_status_rules(self):
		...
		if self.status == "Graduated" and self.quran_status not in {
			...
			frappe.throw(_("A Graduated student must have Quran Status " "Hafiz or Mujaz."))
